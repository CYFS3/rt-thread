                                 ...
                                 rt_bool_t nonblocking)
{
    ...
    {
        LOG_E("CAN %s write failed on %s, size=%d",
...
#ifdef RT_CAN_TC_EXTERNAL_USING_RECEIVE_TEST
static rt_err_t can_receive_fd_patterns(struct can_tc_context *context,
                                        rt_uint32_t id,
                                        rt_uint8_t seed)
{
    rt_err_t result;
    ...
    for (i = 0; i < message_count; i++)
    {
        result = can_receive_expected(context, &messages[i]);
        if (result != RT_EOK)
        {
            return result;
        }
...
#ifdef RT_CAN_TC_USING_CANFD
    result = can_receive_fd_patterns(can_rx_ctx, RT_CAN_TC_RX_FRAME_ID, CAN_TC_RX_SEED);
    if (result != RT_EOK)
    {
        return result;
    }
